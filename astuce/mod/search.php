<?php
/*
astuce/mod/search.php

Module simplifié de recherche d'astuce
*/
	...
	<div class="nf">
		...
		?>
			...
			echo fil_ariane(1, '', 'Accueil').'
				...
				'.fil_ariane(2, 'astuce/?mod=search', 'Module simplifié de recherche d\'astuce');
			?>
			</div>
			...
			{
				echo '<a href="'.ROOTPATH.'/astuce/?mod=search&c='.$value.'">'.$key.'</a><br/>';
			}
		}
		else
		{
			if(!isset($_GET['i']))
			{
				if($_GET['c'] != '')
				{
					...
					'.fil_ariane(2, 'astuce/?mod=search&amp;console='.$console, 'Recherche pour les jeux de '.$console_2);
					?>
					</div>
					...
					{
						echo '<a href="'.ROOTPATH.'/astuce/?mod=search&amp;c='.$console.'&amp;i='.$key.'">'.$key.'</a> | ';
					}
					?>
					</p>
					<?php
				...
				else
				{
					...
					<?php echo fil_ariane(1, '', 'Accueil').'
						'.fil_ariane(1, 'astuce/', 'Interface des astuces').'
						'.fil_ariane(1, 'astuce/?mod=search', 'Module simplifié de recherche d\'astuce').'
						'.fil_ariane(2, 'astuce/?mod=search', 'Erreur -> Lien corrompu');
					?>
					</div>
					<?php
					quit('Lien corrompu');
			...
			else
			{
				if($_GET['i'] != '')
				{
					...
					'.fil_ariane(2, 'astuce/?mod=search&amp;c='.$console.'&amp;i='.$initiale, 'Recherche pour les jeux commençant par '.$initiale);
					?>
					</div>
					...
					<?php
					$sql_listing_jeu_astuce = mysql_query("
					...
					if(mysql_num_rows($sql_listing_jeu_astuce) > 0)
					{
						echo '<br/>';
						echo '<p align="left">';
						while($req_listing_jeu_astuce = mysql_fetch_array($sql_listing_jeu_astuce))
						{
							$sql_id_membre_auteur = mysql_query("SELECT id FROM site_membres WHERE pseudo = '".$req_listing_jeu_astuce['auteur']."'") or ErrorSQL(__FILE__,__LINE__);
							$req_id_membre_auteur = mysql_fetch_array($sql_id_membre_auteur);
							echo '&nbsp;<a href="'.ROOTPATH.'/astuce/a/?c='.$console.'&amp;i='.$initiale.'&amp;id='.$req_listing_jeu_astuce['id'].'">'.$req_listing_jeu_astuce['jeu'].'</a> par <a href="'.ROOTPATH.'/?page=membre&amp;id='.$req_id_membre_auteur['id'].'">'.$req_listing_jeu_astuce['auteur'].'</a> le <strong>'.date('d/m/Y à H:i:s', $req_listing_jeu_astuce['time']).'</strong><br/>';
						}
						echo '</p>';
						echo '<br/>';
					}
					...
					{
						echo '<p align="center">Il n\'y a aucun astuce présente pour la lettre '.$initiale.' sur '.$console_2.'</p>';
					}
				}
				else
				{
